...
TC_TITLES_ZH = {
    ...
    "keyboard-languages": "已安裝的鍵盤語言", "root-status": "越獄狀態",
...
DYNAMIC_ZH = {
    ...
    "Waiting for the owner to enter this build's expected iOS Ads SDK version in the report.": "等待 owner 在報告中輸入本次 Build 預期的 iOS Ads SDK 版本。",
    "The request SDK version was captured; comparison waits for the owner-provided build version.": "已自動擷取 Request SDK 版本；等待 owner 提供本次 Build 版號後進行比對。",
    "Enter this Sample App build's intended iOS Ads SDK version; an exact match passes and a mismatch fails.": "請輸入本次 Sample App Build 預期使用的 iOS Ads SDK 版本；完全相等為 PASS，不相等為 FAILED。",
    "Waiting for the owner to enter this build's expected iOS Argus SDK version in the report.": "等待 owner 在報告中輸入本次 Build 預期的 iOS Argus SDK 版本。",
    "Waiting for the owner to enter this build's expected SDK version in the report": "等待 owner 在報告中輸入本次 Build 預期的 SDK 版本。",
    "Waiting for the owner to enter this build's expected Argus SDK version in the report": "等待 owner 在報告中輸入本次 Build 預期的 Argus SDK 版本。",
    "The Argus SDK version was captured; comparison waits for the owner-provided build version.": "已自動擷取 Argus SDK 版本；等待 owner 提供本次 Build 版號後進行比對。",
    "Enter this Sample App build's intended iOS Argus SDK version; an exact match passes and a mismatch fails.": "請輸入本次 Sample App Build 預期使用的 iOS Argus SDK 版本；完全相等為 PASS，不相等為 FAILED。",
    "Enter this build's intended SDK version; an exact match passes and a mismatch fails.": "請輸入本次 Build 預期使用的 SDK 版本；完全相等為 PASS，不相等為 FAILED。",
    "Enter this build's intended Argus SDK version; an exact match passes and a mismatch fails.": "請輸入本次 Build 預期使用的 Argus SDK 版本；完全相等為 PASS，不相等為 FAILED。",
    ...
    "FAILED: Extended device.ext.iaphistory is present but is not an array of unique non-empty product-ID strings.": "FAILED：Extended device.ext.iaphistory 已存在，但不是由唯一且非空的 product ID 字串組成的陣列。",
    "The Sample App has no observable purchase flow or independent expected product IDs, so purchase-history correctness cannot be judged.": "Sample App 沒有可觀測的購買流程或獨立 expected product IDs，因此無法判定購買紀錄正確性。",
    ...
    "R3 did not prove that both requests used the same iOS App PID; the same-process session comparison was not executed.": "R3 未證明兩份 request 使用同一個 iOS App PID，因此未執行同一 process 內的 session 比較。",
    "Session duration increases while the same App process remains alive.": "在同一個仍存活的 App process 中，session duration 已正確遞增。",
    "FAILED: session_duration did not increase within the proven same App process.": "FAILED：已證明兩份 request 位於同一 App process，但 session_duration 沒有遞增。",
    "R3 termination setup did not prove that the old iOS App process exited and Request 4 used a new PID; the termination-dependent comparison was not executed.": "R3 終止設定未證明舊 iOS App process 已結束且 Request 4 使用新 PID，因此未執行依賴終止前提的比較。",
    "Session duration resets after the old App process exits and a new process starts.": "舊 App process 結束且新 process 啟動後，session duration 已正確重設。",
    "FAILED: session_duration did not reset after a proven App process restart.": "FAILED：已證明 App process 重新啟動，但 session_duration 沒有重設。",
    "R3 did not prove that Requests 1–3 used one iOS App PID and Request 4 used a new PID; the process-scoped initialization comparison was not executed.": "R3 未證明 Request 1～3 使用同一個 iOS App PID，且 Request 4 使用新 PID，因此未執行 process-scoped 初始化時間比較。",
    "app_init_time remains stable in one process and renews after a proven process restart.": "app_init_time 在同一 process 內保持不變，並在已證明的 process 重啟後更新。",
    "FAILED: app_init_time was not stable in one process or did not renew after a proven process restart.": "FAILED：app_init_time 未在同一 process 內保持穩定，或未在已證明的 process 重啟後更新。",
    "R3 did not prove one App PID through background/resume and a new PID after termination; the cross-process app-duration comparison was not executed.": "R3 未證明 background/resume 期間維持同一 App PID，且終止後使用新 PID，因此未執行跨 process 的 app duration 比較。",
    "Today's foreground usage remains monotonic across background and a proven process restart.": "今日前景使用時間在 background 與已證明的 process 重啟前後皆保持單調不減。",
    "FAILED: app_duration decreased during the proven same-day lifecycle sequence.": "FAILED：在已證明的同日 lifecycle sequence 中，app_duration 發生下降。",
    ...
    "Extended device.ext.jailbreak is JSON boolean false; Request is absent or also false.": "Extended 的 device.ext.jailbreak 為 JSON boolean false；Request 未送出此欄位，或同樣為 false。",
    "FAILED: device.ext.jailbreak must be JSON boolean false in Extended and must be absent or false in Request.": "FAILED：Extended 的 device.ext.jailbreak 必須是 JSON boolean false；Request 必須未送出此欄位或為 false。",
    ...
    "XCUITest/WDA did not capture an independent iOS device coordinate before the Bid.": "Bid 前未能透過 XCUITest／WDA 擷取獨立的 iOS 裝置座標。",
    "The decoded coordinate pair agrees with the independent iOS device location within the reviewed accuracy tolerance.": "解碼後的經緯度座標與獨立 iOS 裝置位置一致，且落在審核過的精度容許範圍內。",
    "FAILED: ext.device.geo_lat/geo_lon is missing, invalid, or outside the independent iOS device location tolerance.": "FAILED：ext.device.geo_lat／geo_lon 缺值、格式錯誤，或超出獨立 iOS 裝置位置的容許距離。",
    "Hardware limitation: the iOS QA device has no active SIM; Carrier cannot be captured or verified.": "硬體限制：iOS QA 裝置沒有 active SIM，無法取得或驗證電信業者。",
    "Hardware limitation: the iOS QA device has no active SIM; MCC/MNC cannot be captured or verified.": "硬體限制：iOS QA 裝置沒有 active SIM，無法取得或驗證 MCC/MNC。",
    "An active SIM is present, but an independent iOS Carrier reference is not captured yet.": "已偵測到 active SIM，但尚未擷取獨立的 iOS 電信業者真值。",
    "An active SIM is present, but an independent iOS MCC/MNC reference is not captured yet.": "已偵測到 active SIM，但尚未擷取獨立的 iOS MCC/MNC 真值。",
